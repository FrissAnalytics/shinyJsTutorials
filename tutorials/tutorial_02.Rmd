...
A shiny demo app containing all the widgets can be found [here](https://github.com/FrissAnalytics/shinyJsTutorials/tree/master/tutorials/materials2/C3_demo). To see the app in action press the screenshot below! 


<p style = "margin: 40px 0px">
  <a href = "https://frissdemo.shinyapps.io/GaugeApp/" target = "_blank"><img src = "C3_dashboard.png" width="100%"></img></a>
  <div style = "text-align:center">demo app with animated C3 widgets. Click on the image to launch the app!</div>  
</p>


Below we show you how to create the widgets, which you can create with `C3Gauge`,`C3Pie`,`C3LineBarChart`, and `C3StackedAreaChart`. 


In a shiny app, these functions are paired with the output functions (in ui.r): `C3GaugeOutput`, `C3PieOutput`, `C3LineBarChartOutput`, `C3StackedAreaChartOutput`, and the render functions (in server.R): `renderC3Gauge`, `renderC3Pie`, `renderC3LineBarChart`, `renderC3StackedAreaChart`. 
...
Looking at the fiddle you can see we have a lot more options to deal with. To keep things simple, we're not going parameterize all of them. In the widget below, the user provides a single dataframe called `dataset`, with a time column (used for the x-axis) and a set of additional columns (used as y-values) for the bar and line series. Furthermore, the user must specify the a `colors` variable that holds the display colors for the bar and line charts. 

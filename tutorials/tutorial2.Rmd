...
A shiny demo app containing all the widgets can be found [here](https://github.com/FrissAnalytics/shinyJsTutorials/tree/master/tutorials/materials2/C3_demo). To see the app in action press the screenshot below. Below we show you how to create the widgets, which you can create with `C3Gauge`,`C3Pie`,`C3LineBarChart`, and `C3StackedAreaChart`. In a shiny, these functions are paired with the output functions (in ui.r) `C3GaugeOutput`,`C3PieOutput`,`C3LineBarChartOutput`,`C3StackedAreaChartOutput`, and the render functions (in server.R) `renderC3Gauge`,`renderC3Pie`,`renderC3LineBarChart`,`renderC3StackedAreaChart`. Note that you can also use the widgets in a MarkDown document as well!
...
In the sections below we presented the details of the C3 pie chart, combination line + bar chart and stacked area chart. You can also experiment with code changes right in the browser with the online app discussed in the next section.  
